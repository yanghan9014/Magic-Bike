...
def color_center_dist(RGB_img, depth_img, color):
    boundaries = {
    ...
    if depth_img.size == 307200:
        ...
        color_dep = min(500, color_dep)
        print("Avg color ", color, " depth:", color_dep)
        dist = color_dep / 2.2 * (avg_n - 320) / 320
        print("Estimated horizontal distance:", dist, "mm")
        dist = min(400, dist)
        dist = max(-400, dist)
    right = 1 if dist > 0 else 0
    return (avg_m, avg_n, mask, color_dep, dist, right)
